...
function CountdownTimer({ seconds, onExpire }: { seconds: number; onExpire?: () => void }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setRemaining((s) => {
        if (s <= 1 && !fired.current) { fired.current = true; setTimeout(() => onExpire?.(), 0); }
        return s - 1;
      });
    }, 1000);
  ...
  return (
    <span className={remaining <= 0 ? "text-wine-700/80 text-xs" : "text-olive-400 text-xs"}>
      {remaining <= 0 ? "Código expirado" : `Válido por ${m}:${String(s).padStart(2, "0")} min`}
    </span>
  );
}
...
export function StickyRedeemBar({
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      try {
        const { data } = await createClient().from("redemptions").select("status").eq("id", redemptionId).single();
  ...
  async function saveFinancials() {
    ...
    if (!totalBill || isNaN(total) || total <= 0) { setFormError("Introduza o valor total da fatura."); return; }
    if (discount === "" || isNaN(disc) || disc < 0 || disc > total) { setFormError("Introduza o valor do desconto (entre 0 € e o total da fatura)."); return; }
    setFormError(undefined);
    setPhase("saving");
    try {
      ...
      if (!res.ok) { const json = (await res.json()) as { message?: string }; throw new Error(json.message); }
      setBenefitUsed(true);
      setPhase("saved");
    } catch (err) {
    ...
    return (
      ...
      >
        <div className="mx-auto flex max-w-lg items-center justify-between gap-4">
          ...
          <a href="/registar" className="bg-gold-500 hover:bg-gold-500/90 flex-none rounded-full px-6 py-3 text-sm font-semibold text-olive-900 transition">
            Aderir ao Clube
          </a>
        </div>
      </div>
          ...
          <div className="relative flex justify-center pt-3 pb-1">
            ...
            {phase !== "confirmed" && phase !== "saving" && (
              <button
                ...
                className="absolute right-4 top-0 flex h-7 w-7 items-center justify-center rounded-full bg-olive-900/8 text-olive-700 hover:bg-olive-900/15 transition"
              >
            ...
            {phase === "error" && (
              <div className="py-4">
                ...
                <div className="mt-5 flex gap-3">
                  <button onClick={() => void redeem()} className="flex-1 rounded-full bg-olive-900 py-2.5 text-sm font-semibold text-white">
                    Tentar novamente
                  </button>
                  <button onClick={() => { setOpen(false); setPhase("idle"); }} className="rounded-full border border-olive-900/20 px-4 py-2.5 text-sm font-semibold text-olive-700">
                    Fechar
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="rounded-2xl bg-olive-900 py-7 text-center">
                    ...
                    <p className="mt-2">
                      <CountdownTimer seconds={CODE_TTL} onExpire={() => {
                        if (phase === "code") {
                ...
                <div className="flex items-center gap-3 rounded-xl border border-olive-900/8 bg-cream-50 px-4 py-3">
            ...
            {(phase === "confirmed" || phase === "saving") && (
              <div className="space-y-4">
                <div className="flex items-center gap-2 rounded-xl bg-olive-700/10 px-3 py-2.5">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-olive-700 text-xs text-white">✓</span>
                  <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input type="number" inputMode="decimal" min="0" step="0.01" value={totalBill}
                  ...
                  <div>
                    ...
                    <input type="number" inputMode="decimal" min="0" step="0.01" value={discount}
                ...
                <button onClick={() => void saveFinancials()} disabled={phase === "saving"}
        ...
        <div className="mx-auto flex max-w-lg items-center justify-between gap-4">
          ...
          {benefitUsed ? (
            <div className="flex-none rounded-full bg-olive-900/8 px-5 py-3 text-center">
              <p className="text-xs font-semibold text-olive-500">Benefício utilizado</p>
