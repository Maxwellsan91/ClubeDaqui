...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; className: string }> = {
    PENDING: { label: "Em carência", className: "bg-gold-500/15 text-olive-800" },
    VALIDATED: { label: "Validado", className: "bg-olive-900/10 text-olive-700" },
    CANCELLED: { label: "Cancelado", className: "bg-wine-700/10 text-wine-700" },
  };
  const s = map[status] ?? { label: status, className: "bg-cream-100 text-olive-400" };
  return (
...
export default function InfluencerDetailPage() {
  ...
  async function load() {
    ...
    if (res.ok) {
      const json = (await res.json()) as { data?: InfluencerDetail; error?: string };
      if (json.data) {
        setData(json.data);
        setForm({
    ...
    <div className="space-y-6">
      ...
      <div>
        <a
          ...
          className="mb-4 inline-flex items-center gap-1.5 text-sm text-olive-500 hover:text-olive-900 transition-colors"
        >
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl font-bold text-olive-900">{data.name}</h1>
                <button
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 rounded-xl border border-olive-900/12 px-4 py-2.5">
                ...
                <span className="font-mono text-sm font-bold text-olive-900">{data.uniqueCode}</span>
                <button
                  onClick={() => void navigator.clipboard.writeText(data.uniqueCode)}
                  className="rounded p-0.5 text-olive-400 hover:text-olive-700 transition-colors"
                  title="Copiar"
                >
              ...
              <button
                ...
                className="rounded-xl bg-olive-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-olive-900/90 transition-colors"
              >
                Editar
              </button>
            </div>
          ...
          {/* Details row */}
          <div className="mt-4 flex flex-wrap gap-6 border-t border-cream-100 pt-4 text-sm">
            <div>
              <span className="text-olive-400">Comissão</span>
              <span className="ml-2 font-semibold text-olive-900">{data.commissionRate}%</span>
            </div>
            {data.notes && (
              <div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-olive-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            <h2 className="mb-5 text-base font-bold text-olive-900">Editar influencer</h2>
            <div className="space-y-4">
              <div>
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-olive-900">Notas</label>
                <textarea
                  rows={3}
                  value={form.notes}
            ...
            {saveError && <p className="mt-3 text-sm text-wine-700">{saveError}</p>}
            <div className="mt-5 flex gap-3">
              <button onClick={() => setEditing(false)} className="flex-1 rounded-xl border border-olive-900/15 py-2.5 text-sm font-medium text-olive-700 hover:bg-cream-50">
                Cancelar
              </button>
              <button onClick={() => void saveEdit()} disabled={saving} className="flex-1 rounded-xl bg-olive-900 py-2.5 text-sm font-semibold text-white disabled:opacity-60">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        {[
          ...
          { label: "Comissão devida", value: `${data.totals.validatedCommission.toFixed(2)} €`, color: "bg-olive-900 text-white" },
          { label: "Em processamento", value: `${data.totals.pendingCommission.toFixed(2)} €`, color: "bg-gold-500/15" },
        ].map((s) => (
          <div key={s.label} className={`rounded-2xl p-4 shadow-sm ${s.color || "bg-white"}`}>
            <p className={`text-[10px] font-semibold uppercase tracking-wide ${s.color.includes("olive-900") ? "text-cream-50/60" : "text-olive-400"}`}>
              {s.label}
            </p>
            <p className={`mt-1 text-xl font-bold ${s.color.includes("olive-900") ? "text-gold-500" : "text-olive-900"}`}>
              {s.value}
            </p>
          </div>
        ))}
      ...
      <div className="overflow-hidden rounded-2xl bg-white shadow-sm">
        <div className="border-b border-cream-100 px-5 py-4">
          ...
          <p className="mt-0.5 text-xs text-olive-500">
            Comissão de {data.commissionRate}% por adesão · preço base 24 € · carência 15 dias
          </p>
        </div>
        {data.monthly.length === 0 ? (
          <p className="px-5 py-8 text-center text-sm text-olive-400">Sem registos ainda.</p>
        ) : (
                ...
                {data.monthly.map((m) => (
                  <tr key={m.month} className="hover:bg-cream-50/50">
                    ...
                    <td className="px-4 py-3.5 text-right font-semibold text-olive-900">
                      {m.validatedCommission > 0 ? `${m.validatedCommission.toFixed(2)} €` : "—"}
                    </td>
                    <td className="px-4 py-3.5 text-right text-olive-400">
                      {m.pendingCommission > 0 ? `${m.pendingCommission.toFixed(2)} €` : "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="border-t-2 border-olive-900/10 bg-cream-50/50">
                <tr className="text-sm font-bold text-olive-900">
                  <td className="px-5 py-3">Total</td>
                  <td className="px-4 py-3 text-center">{data.totals.total}</td>
                  <td className="px-4 py-3 text-center">{data.totals.validated || "—"}</td>
                  <td className="px-4 py-3 text-center text-wine-700/70">{data.totals.cancelled || "—"}</td>
      ...
      <div className="overflow-hidden rounded-2xl bg-white shadow-sm">
        ...
        {data.referrals.length === 0 ? (
          <p className="px-5 py-8 text-center text-sm text-olive-400">Nenhum indicado ainda.</p>
        ) : (
          <div className="overflow-x-auto">
              ...
              <tbody className="divide-y divide-cream-100">
                {data.referrals.map((r) => (
                  <tr key={r.id} className="hover:bg-cream-50/50">
                    <td className="px-5 py-3.5 font-semibold text-olive-900">{r.memberName}</td>
                    <td className="px-4 py-3.5 text-olive-600">{fmt(r.createdAt)}</td>
                    <td className="px-4 py-3.5 text-olive-600">
                      {r.status === "CANCELLED" ? (
                        <span className="text-wine-700/60">Cancelado em {r.cancelledAt ? fmt(r.cancelledAt) : "—"}</span>
                      ) : (
                        fmt(r.validatesAt)
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3.5 text-right font-semibold">
                      {r.status === "VALIDATED" ? (
                        <span className="text-olive-900">{r.commission.toFixed(2)} €</span>
                      ) : r.status === "PENDING" ? (
                        <span className="text-olive-400">{r.pendingCommission.toFixed(2)} € *</span>
                      ) : (
                    ...
                    <td className="px-4 py-3.5 text-right">
                      {r.status === "PENDING" && (
                        <button
                          ...
                          className="text-xs font-medium text-wine-700/60 hover:text-wine-700 underline underline-offset-2 transition-colors"
                        >
                          Cancelar
                        </button>
                      )}
            ...
            <div className="border-t border-cream-100 px-5 py-3">
              <p className="text-xs text-olive-400">* Comissão em carência — processada após 15 dias sem cancelamento</p>
            </div>
          </div>
        )}
      </div>
