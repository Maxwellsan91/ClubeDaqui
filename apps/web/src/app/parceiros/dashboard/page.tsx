      ...
      {series.map((s) => {
        ...
        return (
          <div key={s.month} className="flex flex-1 flex-col items-center justify-end gap-1.5">
            {s.count > 0 && (
              <span className="text-[10px] font-bold text-olive-900">{s.count}</span>
            )}
            <div
            ...
            />
            <span className="text-[10px] font-medium text-olive-500">{s.month}</span>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="min-h-screen bg-cream-50">
      ...
      <header className="sticky top-0 z-40 border-b border-olive-900/8 bg-white/95 backdrop-blur-sm">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-4 px-5 py-3 sm:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold tracking-[0.2em] text-olive-500 uppercase">Clube Ribatejo</p>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              <svg className="mr-1.5 h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true">
      ...
      <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8 sm:py-12">
        ...
        {/* Page title */}
        <div className="mb-8">
          <p className="text-[11px] font-bold tracking-[0.28em] text-wine-700 uppercase">Dashboard</p>
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-olive-600">Parceiro ativo</span>
          </div>
        </div>
        ...
        {loadState === "loading" && (
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-24 animate-pulse rounded-2xl bg-olive-900/5" />
            ))}
          </div>
        )}
        ...
        {loadState === "error" && (
          <div className="rounded-2xl border border-dashed border-olive-900/20 bg-white p-8 text-center">
            <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-wine-700/8 text-wine-700">
              <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                <circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" />
              </svg>
            </div>
            <p className="font-semibold text-olive-900">Não foi possível carregar os dados</p>
            <p className="mt-1 text-sm text-olive-600">Verifique a sua ligação e tente novamente.</p>
            <button
          ...
          <div className="space-y-5">
            ...
            <div className="overflow-hidden rounded-2xl bg-olive-900 p-6 sm:p-8">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-olive-300">Faturação total gerada</p>
                </div>
                <div>
                  <p className="font-display text-4xl tracking-tight text-white">
                    {stats.uniqueMembers}
                  ...
                  <p className="mt-1 text-xs text-olive-300">
                    {stats.uniqueMembers === 1 ? "Membro único" : "Membros únicos"}
                  </p>
                </div>
                <div>
                  <p className="font-display text-4xl tracking-tight text-white">
                  ...
                  <p className="mt-1 text-xs text-olive-300">Benefícios utilizados</p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-olive-900/10 bg-white p-5 sm:p-6">
              <div className="mb-5 flex items-start justify-between">
                <div>
                  <p className="text-[11px] font-bold tracking-[0.2em] text-olive-500 uppercase">Tendência mensal</p>
                  <p className="font-display mt-1 text-xl text-olive-900">Visitas nos últimos 6 meses</p>
                </div>
              </div>
            ...
            {stats.recentRedemptions.length > 0 ? (
              <div className="rounded-2xl border border-olive-900/10 bg-white overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 sm:px-6">
                  <div>
                    <p className="text-[11px] font-bold tracking-[0.2em] text-olive-500 uppercase">Atividade recente</p>
                    <p className="font-display mt-0.5 text-lg text-olive-900">Últimas utilizações</p>
                  </div>
                </div>
                  ...
                  {stats.recentRedemptions.map((r) => (
                    <div key={r.id} className="flex items-center gap-3 px-5 py-3.5 sm:px-6">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[11px] text-olive-500">{r.benefitTitle}</p>
                      </div>
                      <div className="shrink-0 text-right">
                        {r.billAmount != null && (
                          <p className="text-sm font-bold text-olive-900">{euro.format(r.billAmount)}</p>
                        )}
                        {r.discountAmount != null && (
                          <p className="text-[10px] text-olive-500">−{euro.format(r.discountAmount)}</p>
                        )}
              ...
              <div className="rounded-2xl border border-dashed border-olive-900/20 bg-white p-8 text-center">
                <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-olive-900/6">
                  <svg className="h-6 w-6 text-olive-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
                    <path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </div>
                <p className="font-display text-xl text-olive-900">Pronto para receber os primeiros membros</p>
                <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-olive-600">
                  Quando um membro apresentar o código de benefício, confirme-o aqui e os dados aparecerão automaticamente.
                </p>
                <Link
                  href="/parceiros/validar"
            ...
            <div className="border-t border-olive-900/10 pt-6 pb-2">
              <p className="text-xs text-olive-500">
                ...
                <Link href="/parceiros" className="font-semibold text-wine-700 underline underline-offset-2">
                  Fale connosco
                </Link>
              </p>
            </div>
