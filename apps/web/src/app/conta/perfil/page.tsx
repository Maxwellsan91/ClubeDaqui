...
export default function PerfilPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      if (!token || !apiUrl) { router.replace("/entrar"); return; }
  ...
  async function saveProfile() {
    ...
    if (json.error) {
      ...
      setProfile((p) => p ? { ...p, fullName: name, phone: phone || null, nif: nif || null } : p);
    }
    setSaving(false);
  }
    ...
    <main className="min-h-screen bg-cream-50">
      ...
      <div className="mx-auto max-w-2xl px-5 py-10 sm:px-8 sm:py-14">
        ...
        <div className="flex items-center gap-3 mb-8">
          <Link
            href="/conta"
            className="flex h-9 w-9 flex-none items-center justify-center rounded-xl border border-olive-900/15 text-olive-600 hover:bg-cream-100"
          >
            ←
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-olive-900">Minha conta</h1>
            <p className="text-sm text-olive-500">Dados pessoais e credenciais de acesso</p>
          </div>
        </div>
        ...
        {loading ? (
          <div className="space-y-4">
            {[72, 48, 48].map((h, i) => (
              <div key={i} style={{ height: h }} className="animate-pulse rounded-2xl bg-white" />
            ))}
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm">
              <h2 className="text-sm font-bold text-olive-900 mb-5">Dados pessoais</h2>
              ...
              {profileMsg && (
                <p className={`mt-4 text-sm ${profileMsg.ok ? "text-olive-700" : "text-wine-700"}`}>
                  {profileMsg.text}
                </p>
              )}
              <div className="mt-5">
            ...
            {/* Change email */}
            <div className="rounded-2xl bg-white p-6 shadow-sm">
              <h2 className="text-sm font-bold text-olive-900 mb-1">Alterar email</h2>
              <p className="text-xs text-olive-500 mb-4">
                Será enviado um link de confirmação para o novo endereço e para o actual.
              </p>
              ...
              {emailMsg && (
                <p className={`mt-3 text-sm ${emailMsg.ok ? "text-olive-700" : "text-wine-700"}`}>
                  {emailMsg.text}
                </p>
              )}
            </div>
            ...
            {/* Change password */}
            <div className="rounded-2xl bg-white p-6 shadow-sm">
              <h2 className="text-sm font-bold text-olive-900 mb-4">Alterar palavra-passe</h2>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
              ...
              {pwMsg && (
                <p className={`mt-3 text-sm ${pwMsg.ok ? "text-olive-700" : "text-wine-700"}`}>
                  {pwMsg.text}
                </p>
              )}
              <div className="mt-5">
